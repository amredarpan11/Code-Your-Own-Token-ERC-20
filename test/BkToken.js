...
contract('BkToken', function (accounts) {
    var tokenInstance;
    it('initializes the contract with correct value', function () {
        return BkToken.deployed().then(function (instance) {
            ...
            assert.equal(name, 'Bk Token', 'has the correct name');
            return tokenInstance.symbol();
        }).then(function (symbol) {
            assert.equal(symbol, 'Bk', 'has the correct symbol');
            return tokenInstance.standard();
        }).then(function (standard) {
            assert.equal(standard, 'Bk Token v1.0', 'has the correct standard');
        })
    });
